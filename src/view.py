...
def generate_view(data_videos: List, controller: Controller):
    title = f'<p style="font-size:60px; text-align:center; font-family: Verdana, sans-serif; color:#727272;"> Character\'s Identification in TV series Friends Using Voice</p>'
    description =f'''
    <p style="font-size:18px; text-align:center; font-family: Verdana, sans-serif; color:#727272;">
    This demo uses voice snippets to identify main characters from the TV series Friends. Previously,
    a voiceprint (Embedding) generated by the TitanNet Large model has been stored for each principal
    ...
    for row, file_set in enumerate(data_videos):
        for col, object in enumerate(file_set):
            ...
            grid[row][col].button(f"Identify",
